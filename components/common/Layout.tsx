...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children, user, onLogout, onNavigate, currentView }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      {/* Sidebar */}
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-theme-sidebar text-white transition-all duration-300 flex flex-col shadow-2xl z-20`}>
        <div className="p-4 flex items-center justify-between border-b border-white/10">
          <div className={`flex items-center gap-3 ${!isSidebarOpen && 'hidden'}`}>
            <Anchor className="text-theme-primary" size={24} />
            <span className="font-bold text-lg">SeaLearn</span>
          </div>
          <button onClick={() => setSidebarOpen(!isSidebarOpen)} className="p-1 hover:bg-white/10 rounded text-theme-sidebar-text hover:text-white">
          ...
          <div className={`flex items-center gap-3 mb-4 ${!isSidebarOpen && 'justify-center'}`}>
            ...
            {isSidebarOpen && (
              <div className="overflow-hidden">
                ...
                <p className="text-xs text-theme-sidebar-text truncate">{user.email}</p>
              </div>
            )}
          </div>
          <button 
            onClick={onLogout}
            className={`w-full flex items-center gap-3 p-2 rounded-lg text-red-400 hover:bg-red-500/10 hover:text-red-300 ${!isSidebarOpen && 'justify-center'}`}
          >
            <LogOut size={20} />
